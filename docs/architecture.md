...
一套**开源免费、单校自建、纯 Web** 的学校排课、调课与代课管理系统,服务对象为全国小学、初中、普通高中、综合高中、中职的**排课管理员**(核心排课)、教务主任(全校监督与有限日常运行)、系统管理员(系统安全)与教师(已发布课表和本人事务)。
...
- 角色采用固定 RBAC，一人可兼多角色，权限取并集；只要拥有排课管理员角色，默认进入管理视角，同时保留本人事务入口。
- 前端入口只用于减少干扰，后端 HTTP 动作边界是最终授权来源。
- 本版本不增加调课、代课或课表发布审批流。排课管理员直接发布，教务主任查看草稿、监督版本并处理获准的日常运行事务。
- 只有系统管理员或排课管理员可以切换当前学期；历史或归档学期默认只读，可查询和复制，不能常规写入或发布。
...
| A2 | 身为排课管理员，首次登录后通过设置向导：确认学校与学期 → 导入或手工录入基础数据 → 配置作息安排 → 完成检查 | MVP |
...
3. 创建学期时只建立中性学期外壳；作息时间表由用户按学校实际情况确认或新增，科目可通过组合 Excel 或手工录入补充。
...
| `user` / `user_role` | 账号与角色 | 本地账号和密码(bcrypt);角色:admin / director(教务主任)/ scheduler(排课管理员)/ teacher；权限取角色并集，不保存侧栏入口偏好 |
...
- **固定角色管理**:仅系统管理员可以新增账号、重设临时密码、启停账号或调整 `admin / director / scheduler / teacher` 固定角色;禁止停用当前管理员、撤销其管理员角色或移除最后一个启用的管理员。教师与登录账号的绑定、导入时批量创建账号也属于账号权限变更。
...
仪表盘为每个角色提供最多三个固定快捷入口：排课管理角色默认进入排课工作台、教学任务和今日看板；教务主任默认进入课表查询、今日看板和版本与发布；教师默认进入课表查询、请假登记和通知。侧边栏的六个功能分组直接展示所有获准页面，不保存个人排序或最近访问状态；工作空间首页总览只面向系统管理员、排课管理员和教务主任，仪表盘仍是所有角色的默认落点。

设置向导不作为持久侧栏入口。全新系统由路由守卫引导管理用户进入；没有学期时，仪表盘空状态提供“前往设置向导”；已有当前学期时，系统管理提供“检查并补全当前学期”。
...
全新管理用户直接进入四个可返回修改的步骤：**① 学校与学期**（只创建中性学期外壳）→ **② 基础数据**（组合 Excel 预览后原子导入，或按顺序手工录入）→ **③ 作息安排**（确认分组、节次、类型、铃声时间和周视图）→ **④ 完成检查**（区分阻断项与建议提醒，并进入教学任务管理）。

当前步骤和正在设置的学期会持久化，刷新或重新登录后按真实数据从首个未完成步骤继续。完成检查为每项问题提供对应的修改入口。系统管理中的“检查并补全当前学期”复用同一流程，保留现有数据且不会创建重复学期。

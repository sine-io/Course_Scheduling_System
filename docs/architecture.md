...
侧边栏
├── 仪表盘(角色固定的最多三个快捷入口位于页面内)
├── 学期准备(学期与作息时间表、校历、基础数据)
...
仪表盘为每个角色提供最多三个固定快捷入口：排课管理角色默认进入排课工作台、教学任务和今日看板；教务主任默认进入课表查询、今日看板和版本与发布；教师默认进入课表查询、请假登记和通知。侧边栏的五个功能分组直接展示所有获准页面，不保存个人排序或最近访问状态。

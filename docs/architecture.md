...
| 实体 | 说明 | 关键字段 |
|---|---|---|
| `school` | 单校设置(单条) | 校名、学制类型(可复选,如完全中学)、Logo、通知设置 |
...
| `class_unit` | 班级 | 年级、班名、学制标签(普/综/技/初中/小学)、专业类别(中职)、班主任、人数 |
...
| `timetable` | 课表版本 | 状态:`draft` / `published` / `archived`;同学期可多份草稿,仅一份 published |
...
| 后端 | **Python 3.12 + FastAPI + SQLAlchemy 2.0 + Alembic + Pydantic v2** | 与 OR-Tools 同语言;FastAPI 自带 OpenAPI 文件(满足 E4);类型完整 |
...
2. 下载 docker-compose.yml + .env 示例,改两个值(管理员密码、校名)
3. 执行 `sudo docker compose up -d`，然后在浏览器打开 `http://<主机IP>` 进入设置向导
```
...
侧边栏:
├── 仪表盘(今日调课与代课看板、待办)
├── 课表查询(按班级、教师或教室/场地,全员可见)
├── 排课作业(排课管理员)
│   ├── 教学任务管理
│   ├── 排课工作台(拖拽界面)
│   └── 版本与发布
├── 调课与代课(请假登记、处理工作台、月报表)
├── 基础数据(学期/作息时间表/教师/班级/科目/教室/场地/导入)
└── 系统管理(账号、备份、设置)
```

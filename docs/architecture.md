...
3. **30 分钟上手**：首次登录从学期页面开始，沿用日常业务页面完成学期、基础数据、作息和校历准备；所有页面使用全国中小学通用教务用语。
...
| A2 | 身为教务主任，首次登录后能创建当前学期，并从学期准备分组进入基础数据、作息和校历页面继续工作 | MVP |
| A3 | 身为教务主任,我能自定义作息时间表(每天节数、每节起止时间、午休、早自习、周三下午不排课等) | MVP |
| A4 | 身为已有集成的维护者,我能继续通过既有 Excel 导入 API 批量写入教师与班级；基础数据页面不提供导入入口 | 兼容 |
...
3. 创建学期时只建立中性学期外壳；作息时间表由用户按学校实际情况确认或新增，科目通过手工录入补充。既有批量导入 API 保留兼容性，但不再提供基础数据页面入口。
...
**D9 后台任务分两条队列(M6-2)**:`default` 只跑自动排课,`ops` 跑导出/备份/恢复/发送邮件与定时任务,各由一个 worker 进程监听(同一个镜像)。理由是快慢任务不该互相堵住——一次 60 班排课会占住 worker 好几分钟,而那正是教务主任最常按「导出课表」的时候;单一队列下导出会排在排课后面直到超时失败。排课永远只走 `default`,故 `worker-ops` 不加载求解引擎,内存预算低得多(1536MB vs 512MB)。
...
3. 在浏览器打开 `http://<主机IP>`，先创建当前学期，再从学期准备分组维护基础数据、作息和校历
```
...
侧边栏
├── 工作空间(首页总览；限内置系统管理员和教务主任)
├── 学期准备(仪表盘、学期与作息时间表、校历、基础数据)
├── 排课主流程(教学任务、自动排课、排课工作台、版本与发布、课表查询)
...
仪表盘为每个角色提供最多三个固定快捷入口：教务主任和内置系统管理员默认进入排课工作台、教学任务和今日看板；教师默认进入课表查询、请假登记和通知。侧边栏的六个功能分组直接展示所有获准页面，不保存个人排序或最近访问状态；工作空间首页总览只面向内置系统管理员和教务主任，仪表盘仍是所有角色的默认落点。

系统不维护独立的首次设置页面。没有学期时，仪表盘空状态进入“学期与作息时间表”；创建当前学期后，学期准备分组中的基础数据、作息和校历页面始终可以独立进入并返回修改。
...
全新管理用户从“学期与作息时间表”创建仅含学年、学期和真实起止日期的中性学期，再通过侧边栏进入“基础数据”“学期与作息时间表”和“校历与排课准备”。这些是持续使用的真实业务页面，不保存另一套步骤进度或完成状态。

基础数据按科目、教师、班级、教室/场地的引用顺序提供手工维护；作息页面维护分组、节次、类型、铃声时间和周视图；校历页面维护特殊日期并执行排课准备确认。仪表盘只展示当前学期数据摘要，自动排课页面的前置检查负责判断具体排课数据是否可用。

...
设置向导不作为持久侧栏入口。全新系统由路由守卫引导管理用户进入；没有学期时，仪表盘空状态提供“前往设置向导”，系统管理保留重新执行入口。
...
全新管理用户直接进入五个可返回修改的步骤：**① 选择“初中（空白模板）”**（带入科目参考项和空白作息时间表）→ **② 设置学年学期** → **③ 填写作息时间表** → **④ 下载 Excel 模板并导入教师/班级** → **⑤ 完成并进入教学任务管理**。

当前步骤和正在设置的学期会持久化，刷新或重新登录后从中断处继续。系统管理员和排课管理员可以跳过向导，也可以从系统管理重新启动；重新启动不会删除现有数据。

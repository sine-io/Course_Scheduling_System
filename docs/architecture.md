...
一套**开源免费、单校自建、纯 Web** 的学校排课、调课与代课管理系统,服务对象为全国小学、初中、普通高中、综合高中、中职的**教务主任**(全部学校业务)、内置系统管理员(系统安全)与教师(已发布课表和本人事务)。

三大设计原则(所有后续决策的最高准则):

1. **不写死学制**：不同学校的差异统一表达为可配置数据（作息时间表、排课单位和约束），不在业务代码中增加地区或学制分支。公开初始化不提供学校级学制模板；新学期保持中性，作息建议只按已有班级的学制标签分组并由用户确认。
2. **一键部署**：执行 `sudo docker compose up -d` 即可完成安装；备份、恢复和升级均提供界面按钮或单行命令。
3. **30 分钟上手**：首次登录从排课工作台的学期创建卡片开始，在工作台的学期管理、校历与准备、资源与导入辅助工作面完成准备；所有页面使用全国中小学通用教务用语。
...
- 角色采用固定 RBAC；系统管理员仅属于唯一内置账号，普通账号可兼任教务主任和教师，权限取并集；只要拥有教务主任角色，默认进入管理视角，同时保留本人事务入口。
- 前端入口只用于减少干扰，后端 HTTP 动作边界是最终授权来源。
- 本版本不增加调课、代课或课表发布审批流。教务主任直接发布并承担全部学校业务，内置系统管理员保留系统级管理职责。
- 只有系统管理员或教务主任可以切换当前学期；历史或归档学期默认只读，可查询和复制，不能常规写入或发布。
...
| # | User Story | 优先级 |
|---|---|---|
| A1 | 身为信息教务主任,我能用一行 Docker 指令架好系统,并用默认管理员账号登录 | MVP |
| A2 | 身为教务主任，首次登录后能创建当前学期，并在排课工作台的辅助工作面完成基础数据、作息和校历准备 | MVP |
| A3 | 身为教务主任,我能自定义作息时间表(每天节数、每节起止时间、午休、早自习、周三下午不排课等) | MVP |
| A4 | 身为已有集成的维护者,我能继续通过既有 Excel 导入 API 批量写入教师与班级；工作台资源与导入辅助面不改变接口和权限边界 | 兼容 |
| A5 | 身为教务主任,我能维护教室/场地(普通教室、专用教室、实训场地,含容量与适用科目) | MVP |
| A6 | 身为管理员,我能批量创建教师账号并发送启用通知 | MVP |
| A7 | 身为教务主任,我能开新学期并复制上学期的基础数据(教师/班级/科目/作息时间表) | MVP |
...
| # | User Story | 优先级 |
|---|---|---|
| B1 | 身为教务主任,我能创建教学任务:排课单位 × 科目 × 教师(可多位协同)× 每周节数 × 连堂规则 × 教室/场地需求 | MVP |
| B2 | 教务主任可以创建走班分组（多班联排，如高中选修课程），并让分组内课程在同一时段开课 | MVP |
| B3 | 身为教务主任,我能看到每位教师的教学任务节数统计,实时对照基本课时(超课时/不足以颜色标示) | MVP |
| B3-1 | 身为教务主任,我能设置允许的超课时上限；教师教学任务总量超过“基本课时减行政减课数再加上限”时，创建、修改与 Excel 导入均被阻止并说明教师姓名。未维护基本课时(`base_periods=0`)的教师不执行此限制 | v1.2 |
| B4 | 身为教务主任,我能用 Excel 批量导入教学任务数据 | MVP |
| B5 | 身为教务主任,我能设置教师「不可排课时段」(兼行政、公假、培训)与「偏好时段」 | MVP |

#### C. 排课

| # | User Story | 优先级 |
|---|---|---|
| C1 | 身为教务主任,我能在周课表格子上以拖拽方式手动排课,冲突单元格会实时以红色提示并说明原因 | MVP |
| C2 | 身为教务主任,我能切换班级、教师、教室/场地三种课表视角 | MVP |
| C3 | 身为教务主任,我能一键自动排课,看到进度,并在完成后看到软约束达成度报告 | MVP |
| C4 | 身为教务主任,我能「锁定」部分单元格(如已敲定的任课教师时段),自动排课不得移动 | MVP |
| C5 | 身为教务主任,自动排课无解时,系统告诉我最可能冲突的约束(如「王老师教学任务 22 节但可排时段仅 20 格」) | MVP |
| C6 | 身为教务主任,我能保留多个课表草稿版本,比较后选一个发布 | MVP |
| C7 | 身为教务主任,发布后全校师生可查询课表 | MVP |
| C8 | 身为教务主任,我能对已发布课表做局部调整并重新发布(保留历次版本) | MVP |
| C9 | 身为教务主任,我能设置高级软约束权重(科目分布均匀、教师空堂集中等) | v2 |
...
| D2 | 身为教务主任,我能替教师代登请假 | MVP |
| D3 | 身为教务主任,对每一受影响节次,我能选择处理方式:代课/调课(对调)/合班/自习/公假不处理 | MVP |
| D4 | 身为教务主任,系统推荐可代课教师(该时段空堂 → 同科目优先 → 当日已在校 → 代课课时平衡),一键指派 | MVP |
| D5 | 身为被指派教师,我会收到站内通知与 Email,可一键「确认收到」(实际工作中,教务主任指派前已口头征得同意,因此**不设置拒绝流程**,2026-07-09 用户确定) | MVP |
| D6 | 身为教务主任,我能看到每条代课通知的确认状态,对未确认者一键再次提醒 | MVP |
| D7 | 身为教务主任,我能查看任一日期的「今日调课与代课看板」,打印当日调课与代课通知单 | MVP |
| D8 | 身为教务主任,我能设置调课(甲乙教师对调两节课),系统验证双方均无冲突 | MVP |
| D9 | 身为教务主任,月底我能导出代课课时统计表(依教师、依请假类型、依经费来源) | MVP |
...
> 结论：学校差异收敛为四类可配置机制——**自定义作息时间表、弹性排课单位（班级/走班群组）、连堂规则和可排时段限制**。系统不提供学校级学制模板；常用科目可在手工录入中逐项勾选，作息必须通过可见参数和预览确认后写入。
...
3. 创建学期时只建立中性学期外壳；作息时间表由用户按学校实际情况确认或新增，科目通过手工录入补充。既有批量导入 API 保留兼容性，用户入口统一收纳到排课工作台的资源与导入辅助面。
...
`semester.status` 只描述准备中、进行中和已归档等生命周期；单例表 `semester_context` 的 `current_semester_id` 表示学校唯一的日常工作边界。首个学期自动成为当前学期；归档或删除当前学期时指针可暂时为空。只有系统管理员和教务主任可以切换当前学期，客户端携带 `revision` 做乐观并发校验，服务端在切换与常规写入事务中锁定单例行，避免切换期间把结果写入旧学期。只有当前且未归档的学期允许常规编辑、导入、排课和发布；历史或归档学期保留查询及复制到新学期的能力。前端只读状态用于明确反馈，后端校验始终是最终边界。
---
...
- 旋钮 = 教务主任改得动的东西:`H4` 某位教师的不可排时段、`H3` 某个教室/场地的互斥、`H10` 全校的每日科目上限、`H9` 锁定的单元格。
...
前端构建为静态文件由 Caddy 直接服务(不需 Node 容器),共 **6 个容器**:caddy(web)、api、worker、worker-ops、postgres、redis。后三个后端服务统一使用 `ghcr.io/sine-io/course_scheduling_system-backend` 镜像，分别以 `api`、`worker`、`worker ops` 命令启动。

**D9 后台任务分两条队列(M6-2)**:`default` 只跑自动排课,`ops` 跑导出/备份/恢复/发送邮件与定时任务,各由一个 worker 进程监听(同一个 backend 镜像)。理由是快慢任务不该互相堵住——一次 60 班排课会占住 worker 好几分钟,而那正是教务主任最常按「导出课表」的时候;单一队列下导出会排在排课后面直到超时失败。排课永远只走 `default`,`worker-ops` 虽然携带相同的依赖层，却不执行求解路径，内存预算仍低得多(1536MB vs 512MB)。
...
3. 在浏览器打开 `http://<主机IP>`，先进入排课工作台创建当前学期，再从其中的辅助工作面维护基础数据、作息和校历
```
...
- **固定角色管理**:系统管理员是首次初始化创建的唯一内置账号,在账号列表中可见,但不能从账号权限页面编辑、停用或调整角色;`admin` 角色也不能授予其他账号。仅系统管理员可以新增普通账号、重设临时密码、启停账号或调整 `director / teacher` 固定角色。教师与登录账号的绑定属于账号权限变更。
...
- **高风险命令边界**:备份、恢复、核心数据永久/级联删除及账号权限变更仅限系统管理员。每次请求携带一次性的 UUID 操作号、明确确认值和与服务端计算结果完全相同的目标标识;服务端不信任前端隐藏状态，并以唯一操作号拒绝重复提交。权限、确认、目标与幂等校验通过后先记录全局 `pending` 审计，再进入目标业务校验;历史或归档学期在任何核心业务写入前仍执行当前学期只读校验，拒绝时只更新审计而不写业务数据。特殊日期条目维护、草稿课表格移除、代课处理撤回和排课准备确认撤回属于可重建的日常操作，不纳入核心数据删除。界面必须显示具体目标和影响，取消确认不得发送命令。
- **审计**:`audit_log` 记录操作者账号快照、执行时角色、动作、目标、结果、原因和时间。高风险命令先记录 `pending`，最终落为 `success / rejected / failed`；权限不足、确认缺失、目标不匹配、重复请求和业务规则拒绝同样可查询。教师个人信息最小化搜集(不收身份证全码);HTTPS 由 Caddy 处理。
- **备份恢复**:每日 02:00 自动 pg_dump 至 volume(保留 30 份);管理 UI 提供「立即备份、下载、上传恢复」;文件教学 NAS 调度二次备份。恢复前自动创建保护备份，`pg_restore --single-transaction` 保证异常时不提交半完成数据库；非法上传不落盘，失败的临时上传会清除，成功恢复后重建审计记录并强制全员重新登录。
...
侧边栏
├── 工作空间(仪表盘、排课工作台)
├── 排课主流程(课程表调整、版本与发布、课表查询)
├── 日常运行(请假、调课与代课、今日看板、记录、通知)
├── 报表(代课课时统计)
└── 系统管理(系统管理、备份恢复、账号权限；限系统管理员)

独立页面:登录、首次登录强制修改密码和 A4 公告打印。
```

仪表盘为每个角色提供最多三个固定快捷入口：教务主任和内置系统管理员默认进入排课工作台、课程表调整和版本与发布；教师默认进入课表查询、请假登记和通知。侧边栏的五个功能分组直接展示所有获准页面，不保存个人排序或最近访问状态；仪表盘是所有角色的默认落点。

系统不维护独立的首次设置页面。没有学期时，仪表盘空状态进入“排课工作台”创建学期；创建当前学期后，学期管理、校历与准备、资源与导入辅助工作面始终可以从工作台进入并返回修改，旧地址只作兼容重定向。
...
全新管理用户从“排课工作台”创建仅含学年、学期和真实起止日期的中性学期，再在五个步骤中维护班级、作息、科目、教师和教学任务。工作台中的学期管理、校历与准备、资源与导入辅助工作面承担不重复的导入、资源、学期生命周期和特殊日期职责；这些工作面不保存另一套步骤进度或完成状态。

排课工作台按班级、课时、科目节数、教师任课和开始排课的顺序提供唯一维护工作面，并在最后一步运行前置检查。资源与导入辅助面保留组合与来源导入、教室/场地和管理员专用的教师账号绑定；校历与准备辅助面维护特殊日期并提供明确的排课就绪确认和撤销，开始排课动作仍沿用自动确认。仪表盘只展示当前学期数据摘要，课程表调整和版本与发布承接生成后的工作。
...
> **2026-07-09 决策：不设置“邀请/婉拒”流程。** 教务主任在线下确认代课安排后在系统中登记，系统通知用于正式告知。指派立即生效；被指派教师可点击“确认收到”，该状态仅表示已读，不改变课程安排。教务主任可以在看板查看确认情况并再次提醒。

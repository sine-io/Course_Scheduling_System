...
| `semester` | 学年学期 | 学年起始年（如 2026）、学期（1/2）、起止日期、状态（准备中/进行中/已归档）、`is_demo`；示例与正式学期可同学年学期，正式学期之间仍唯一 |
...
正式学期存在后，示例学期不能重新成为当前工作上下文；示例课表仍可查看，但不会覆盖正式学期的首次成功状态或 P0 待办。
示例与正式学期可以共用学年/学期键，数据库只对 `is_demo=false` 的正式记录建立唯一索引，避免首次体验后按当前默认学年创建正式学期时发生冲突。
...
侧边栏
├── 常用(最多五项；角色默认 + 首次成功阶段 + 用户固定/排序 + 最近访问补位)
└── 完整功能
    ├── 学期准备(仪表盘、上手指南、学期与作息时间表、校历、基础数据)
    ├── 排课主流程(教学任务、自动排课、排课工作台、版本与发布、课表查询)
    ├── 日常运行(请假、调课与代课、今日看板、记录、通知)
    ├── 报表(代课课时统计)
    └── 系统管理(学校信息、账号与角色、备份恢复、操作审计、SMTP、排课参数；限系统管理员)

独立页面:登录、首次登录强制修改密码、设置向导和 A4 公告打印。
```

首次发布前，排课管理员的默认常用入口为当前待办、教学任务、自动排课、排课工作台、版本与发布；首次发布后切换为仪表盘、课表查询、今日看板、调课与代课、版本与发布。教务主任、教师和系统管理员使用各自稳定的角色默认入口，完整功能目录仍保留所有获准页面。

首次设置向导中的“示例数据体验”是系统管理页之外的首次入口：排课管理员和系统管理
员选择示例路线后可以创建隔离的示例学期；系统管理页中的再次使用入口仍仅限系统管理
员，且任何正式学期存在时两处都拒绝写入。
...
- **体验示例数据**：创建隔离的示例学期，直接进入自动排课并查看示例发布结果；示例发布不改变正式首次成功状态。
- **正式建校**：进入五个可返回修改的步骤：**① 选择“初中（空白模板）”**（带入科目参考项和空白作息时间表）→ **② 设置学年学期** → **③ 填写作息时间表** → **④ 下载 Excel 模板并导入教师/班级** → **⑤ 完成并进入基础数据**。

路线、当前步骤和正在设置的学期都持久化，刷新或重新登录后从中断处继续。设置向导完成只表示基础配置流程结束；正式首次成功仍须由完整性检查、必排教学任务无遗漏和已发布课表共同推导。

...
- **高风险命令边界**:备份、恢复、核心数据删除及账号权限变更仅限系统管理员。每次请求携带一次性的 UUID 操作号、明确确认值和与服务端计算结果完全相同的目标标识;服务端不信任前端隐藏状态，并以唯一操作号拒绝重复提交。历史或归档学期仍先执行当前学期只读校验。界面必须显示具体目标和影响，取消确认不得发送命令。

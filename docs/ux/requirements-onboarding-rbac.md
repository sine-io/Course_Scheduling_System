...
让没有使用经验的排课管理员通过透明的四步设置向导建立学校的排课基础；同时让教师和教务主任只看到与职责相关的入口。
...
- 全新管理用户进入后依次完成“学校与学期、基础数据、作息安排、完成检查”四个步骤，不显示路线选择，也不按学校类型隐式生成数据。
- 第一步只创建带真实起止日期的中性学期；基础数据可通过先预览后统一提交的组合工作簿导入，也可按引用顺序手工录入。
- 向导保存最后访问步骤和正在设置的学期，完成进度由真实数据推导；保存退出后，仪表盘显示首个未完成步骤并可继续。
- 完成检查区分阻断项和建议项；完成只表示可以开始维护教学任务，不等于排课准备完成或课表已发布。
- 设置向导不作为持久侧栏入口；无学期时仪表盘显示“前往设置向导”，系统管理提供“检查并补全当前学期”，且不复制或新建学期。
...
1. 全新系统进入向导后直接显示“学校与学期”，并明确说明继续后不会自动生成科目、教师、班级或作息。
2. 向导中断后能按真实数据恢复到首个未完成步骤，完成检查通过后进入教学任务管理。

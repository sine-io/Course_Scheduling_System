...
| M7 v1.2 易用性 | 降低首次安装与体验成本 | 一键安装、国内示例数据、学校信息设置、超课时上限和离线文档 |
...
### [x] M1-4 设置向导
- **描述**:首次登录(无任何学期数据时)自动进入五步骤向导(architecture.md §5.2);每步可跳过/回上一步;完成后导向教学任务管理;之后可从系统管理重新启动向导。
- **模块**:`frontend/src/views/wizard/`、`app/api/wizard.py`(进度状态)
- **验收标准**:
  1. 全新系统登录即进入向导；完成五步后，仪表盘显示数据摘要（N 位教师、N 个班级）
  2. 中途关浏览器,再登录从上次步骤继续
  3. 用户测试：不看文档可在 30 分钟内完成初中空白模板、作息时间和基础数据设置
...
- 【Fable 5 总体检 E】列表查询未分页 → **M6-5 已加服务器端上限 1000(下到 SQL)**,`audit-logs` 本来就有 limit;**完整分页 UI 仍列 v1.2**(单校规模下上限已足够)。

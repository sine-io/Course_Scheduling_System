...
### [x] M1-1 学期与作息时间表
- **描述**：`semester` / `period_table` / `period` CRUD（model + API + UI）；作息时间表可视化编辑器；新学期保持中性，不使用学校级学制模板；同一学期可维护多套作息时间表。
- **模块**:`app/models/{semester,period}.py`、`app/api/semesters.py`、`frontend/src/views/settings/PeriodTable.vue`
- **验收标准**:
  1. 可创建中性“2026-2027学年第一学期”，且不会隐式生成科目、节次、铃声、周课时或校历
  2. 可将周五第 7 节改为「班主任时间」,排课时段检查 API 即反映
  3. 可为同学期建第二套作息时间表并指派给不同班级群
- **测试方式**:pytest CRUD + 中性创建;Vitest 组件测试
...
- **描述**:`/settings/system` 只剩左侧菜单,内容区一片空白——**备份、恢复、SMTP、学校级设置四项功能全都点不进去**。`System.vue` 调用 `useDialog()`(M5 复审 fca20ca 加的,用来显示恢复后的可忽略警告),但 `App.vue` 从来没挂 `<n-dialog-provider>`;Naive 会在 setup 直接抛出异常,整页渲染不出来。
...
- **描述**：历史方案曾设想从首次使用入口或系统管理加载虚构的“海州市启明实验初级中学”，包含 18 个班、49 位教师、16 个科目、252 条教学任务和每周 594 课时。课程、班级、职务、作息和时区均采用国内通用表达；所有数值仅供功能演示，不代表政策标准。
- **安全边界**：该历史方案从未成为当前运行时能力；最新产品不提供示例学校或示例数据加载入口。
...
3. **测试金字塔**:pytest 单元(服务层/引擎)为主体;Vitest 覆盖 TimetableGrid 等核心组件;Playwright 仅覆盖六大关键旅程(登录、学期准备、手排、自排、调课与代课、导出)。
...
- 【Fable 5 总体检 E】列表查询未分页 → **操作审计已于 M7-5 完成服务器端分页**；`leaves` 与 `substitution-log` 仍保留 M6-5 的 1000 条服务器上限，并作为后续分页列表中的两项明确待办。

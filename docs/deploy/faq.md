...
**Q:忘记管理员密码怎么办?**
若你还记得 `.env` 的初始 `ADMIN_PASSWORD`,那是**首次登录**用的;登录后改过的密码存在数据库。若连改过的都忘了,在部署主机执行下面的恢复命令,按提示输入新密码:

```bash
sudo docker compose exec -it api python -m app.scripts.recover_admin
```

命令只恢复唯一的内置系统管理员,会解除登录锁定、让已有会话失效,并要求下一次登录修改密码;操作会写入审计。请妥善保管管理员密码。

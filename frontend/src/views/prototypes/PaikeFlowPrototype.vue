...
// PROTOTYPE ONLY: Question: can a compressed scheduling flow make the next
// action obvious without a cross-page wizard? Three variants live at /a, /b, /c.
...
type StageKey = 'prepare' | 'assignments' | 'schedule' | 'review' | 'publish'
...
const stageMeta: Array<{ key: StageKey; label: string; short: string }> = [
  ...
  { key: 'publish', label: '版本发布', short: '发布' },
]
...
const selectedIssue = ref<'assignments' | 'preflight' | 'unscheduled' | 'conflict'>('assignments')
...
const state = reactive<PrototypeState>({
  ...
  preflightWarnings: 2,
  draftStatus: '草稿',
  lastAction: '已载入示范学期与初版课表',
  activity: [
    { id: 1, time: '09:24', text: '初版课表已创建，保留 6 个锁定课位', tone: 'blue' },
...
const currentVariant = computed<VariantKey>(() => {
  const namedVariant = route.name === 'paike-flow-prototype-b'
    ? 'B'
    : route.name === 'paike-flow-prototype-c' ? 'C' : 'A'
  const value = String(route.params.variant ?? route.query.variant ?? namedVariant).toUpperCase()
  return value === 'B' || value === 'C' ? value : 'A'
})

const currentVariantMeta = computed(() => variants.find((item) => item.key === currentVariant.value) ?? variants[0])
const currentStageIndex = computed(() => stageMeta.findIndex((item) => item.key === state.stage))
...
const stageState = (key: StageKey): 'done' | 'active' | 'blocked' | 'idle' => {
  const index = stageMeta.findIndex((item) => item.key === key)
  if (key === 'publish' && state.draftStatus !== '已发布') return 'idle'
  if (index < currentStageIndex.value || (key === 'schedule' && state.runState === 'done')) return 'done'
  if (key === state.stage) return 'active'
...
function chooseVariant(key: VariantKey) {
  ...
  void router.replace({
    path: key === 'A' ? '/prototype/paike-flow' : `/prototype/paike-flow/${key.toLowerCase()}`,
    query,
  })
...
function goToStage(stage: StageKey) {
  state.stage = stage
  record(`查看「${stageMeta.find((item) => item.key === stage)?.label ?? stage}」`, 'blue')
}
...
function applyBatchPanel() {
  ...
  record('已复制上一班配置并补齐 8 项教学任务', 'green')
}

function runPreflight() {
  state.preflightErrors = 0
  state.preflightWarnings = Math.max(0, state.preflightWarnings - 1)
  record('前置检查完成：无阻断错误，剩 1 条提醒', 'green')
}

function openWorkbench() {
  state.stage = 'review'
  record('打开检查调整工作台', 'blue')
}
...
function runAutoSchedule(scope: 'all' | 'subject' = 'all') {
  ...
  runTimer.value = window.setTimeout(() => {
    ...
    state.remaining = 4
    state.conflicts = 1
    record(scope === 'all' ? '自动排课完成：98% 课时已安排' : '数学科目重排完成：剩 4 节待处理', 'green')
    runTimer.value = null
  }, 850)
}
...
function selectIssue(issue: typeof selectedIssue.value) {
  ...
  record(`查看问题：${labels[issue]}`, 'orange')
}
...
function confirmPublish() {
  ...
  state.remaining = 0
  state.conflicts = 0
  record('已发布「春季学期 · 初版课表」并生成站内通知', 'green')
}
...
watch(() => route.query.variant, () => {
  // The URL is the source of truth so each variant has a shareable address.
})
  ...
  <div class="prototype-app" data-testid="paike-flow-prototype">
    <aside class="prototype-sidebar" aria-label="原型导航">
      ...
      <nav class="prototype-nav">
        <p class="prototype-nav-label">工作空间</p>
        ...
        <a class="prototype-nav-link" href="#" @click.prevent="record('查看排课规则入口', 'neutral')">
          <Settings2 :size="17" />
          <span>排课规则</span>
        </a>
        <a class="prototype-nav-link" href="#" @click.prevent="record('查看版本发布入口', 'neutral')">
          <History :size="17" />
          <span>版本与发布</span>
        </a>
    ...
    <div class="prototype-main">
      ...
      <main class="prototype-content">
        <div class="prototype-context-line">
          <span>设计问题：怎样把复杂排课压缩成一条可继续的工作路径？</span>
        ...
        <section v-if="currentVariant === 'A'" class="prototype-variant variant-a" data-testid="variant-a">
          <header class="variant-page-head">
            ...
            <div class="variant-head-actions">
              ...
              <button type="button" class="button button-secondary" @click="openPublishPanel">
                <FileCheck2 :size="15" /> 检查并发布
              </button>
            </div>
          </header>
          ...
          <div class="a-command-grid">
            <section class="command-panel" aria-labelledby="command-title">
              ...
              <h2 id="command-title">{{ state.assignmentReady < state.assignmentTotal ? '先补齐教学任务，再开始排课' : state.stage === 'review' ? '自动排课已完成，检查 4 节待处理课时' : '教学任务已就绪，可以开始自动排课' }}</h2>
              <p v-if="state.assignmentReady < state.assignmentTotal">系统已定位缺项，不需要先浏览其他设置页面。</p>
              <p v-else-if="state.stage === 'review'">从未排列表开始处理，锁定的课位不会被移动。</p>
              <p v-else>前置检查会在进入求解前完成，结果页可直接进入网格调整。</p>
              <div class="command-primary-row">
                ...
                <button v-else-if="state.stage === 'review'" type="button" class="button button-primary" @click="openWorkbench">
                  <Table2 :size="16" /> 打开检查工作台
                </button>
                <button v-else type="button" class="button button-primary" @click="runAutoSchedule('all')">
                  <Play :size="16" /> 开始全校自动排课
                </button>
                <button type="button" class="button button-quiet" @click="goToStage('assignments')">
                  查看全部任务 <ArrowUpRight :size="14" />
                </button>
              </div>
              <div class="command-trust-row">
                <span><CheckCircle2 :size="14" /> 已自动保存</span>
                <span><LockKeyhole :size="14" /> {{ state.locked }} 个课位已锁定</span>
                <span><Zap :size="14" /> 服务端硬约束校验</span>
              </div>
            </section>
            ...
            <aside class="a-state-panel" aria-label="当前状态">
              ...
              <dl class="compact-stats">
                ...
                <div><dt>前置提醒</dt><dd class="is-orange">{{ state.preflightWarnings }}</dd></div>
              </dl>
          ...
          <div class="a-lower-grid">
            <section class="preview-panel">
              ...
              <div class="mini-timetable">
                ...
                <div v-for="row in [{ name: '高一 1 班', cells: ['语文','数学','英语','物理','历史'] }, { name: '高一 2 班', cells: ['数学','英语','化学','语文','体育'] }, { name: '高二 3 班', cells: ['物理','语文','数学','英语','生物'] }]" :key="row.name" class="mini-timetable-row"><strong>{{ row.name }}</strong><span v-for="cell in row.cells" :key="`${row.name}-${cell}`" :class="{ 'is-alert': cell === '物理' }">{{ cell }}</span></div>
              </div>
        ...
        <section v-else-if="currentVariant === 'B'" class="prototype-variant variant-b" data-testid="variant-b">
          <header class="variant-page-head">
            ...
            <div class="variant-head-actions"><button type="button" class="button button-secondary" @click="openPublishPanel"><FileCheck2 :size="15" /> 发布检查</button><button type="button" class="button button-primary" @click="runAutoSchedule('subject')"><WandSparkles :size="15" /> 局部重排</button></div>
          </header>
          ...
          <div class="inbox-layout">
            <aside class="issue-inbox" aria-label="待处理问题">
              ...
              <button type="button" class="issue-row" :class="{ 'is-selected': selectedIssue === 'conflict' }" @click="selectIssue('conflict')"><span class="issue-icon is-purple"><Users :size="16" /></span><span><strong>场地冲突</strong><small>跳转到场地视角定位</small></span><b>{{ state.conflicts }}</b></button>
              <div class="inbox-progress"><div><span>发布准备度</span><strong>{{ state.draftStatus === '已发布' ? '100' : state.completion }}%</strong></div><div class="health-meter"><span :style="{ width: `${state.draftStatus === '已发布' ? 100 : state.completion}%` }" /></div></div>
            </aside>

            <section class="issue-detail" aria-live="polite">
              <div class="detail-topline"><span class="detail-breadcrumb">问题收件箱 / {{ selectedIssue === 'assignments' ? '教学任务缺项' : selectedIssue === 'preflight' ? '排课提醒' : selectedIssue === 'unscheduled' ? '未排课时' : '场地冲突' }}</span><span class="detail-time"><Clock3 :size="13" /> 更新于 09:24</span></div>
              <div class="detail-heading"><span class="detail-icon" :class="`is-${selectedIssue === 'assignments' ? 'red' : selectedIssue === 'preflight' ? 'orange' : 'blue'}`"><CircleAlert :size="22" /></span><div><h2>{{ selectedIssue === 'assignments' ? '还有 8 项教学任务没有完整信息' : selectedIssue === 'preflight' ? '有 2 条规则值得在排课前确认' : selectedIssue === 'unscheduled' ? '还有 14 节课没有找到合适时段' : '机房在周二第 3 节被两个班同时占用' }}</h2><p>{{ selectedIssue === 'assignments' ? '缺少教师或场地的任务无法进入求解器，系统已经按年级聚合。' : selectedIssue === 'preflight' ? '这些提醒不会阻止生成，但处理后更容易得到均衡课表。' : selectedIssue === 'unscheduled' ? '可以只重排数学科目，不影响已经锁定的 6 个课位。' : '从场地视角查看关联课程，调整后会实时重新检查冲突。' }}</p></div></div>
              <div class="detail-action-bar"><button v-if="selectedIssue === 'assignments'" type="button" class="button button-primary" @click="openBatchPanel"><Copy :size="16" /> 批量补齐任务</button><button v-else-if="selectedIssue === 'preflight'" type="button" class="button button-primary" @click="runPreflight"><CheckCircle2 :size="16" /> 处理提醒</button><button v-else-if="selectedIssue === 'unscheduled'" type="button" class="button button-primary" @click="runAutoSchedule('subject')"><WandSparkles :size="16" /> 仅重排数学</button><button v-else type="button" class="button button-primary" @click="record('已打开场地视角定位冲突', 'blue')"><Table2 :size="16" /> 打开场地视角</button><button type="button" class="button button-quiet" @click="record('已标记当前问题稍后处理', 'neutral')">稍后处理</button></div>
              <div class="detail-evidence"><div class="evidence-header"><span>影响范围</span><button type="button" class="prototype-text-button" @click="advancedRulesOpen = !advancedRulesOpen"><Settings2 :size="14" /> {{ advancedRulesOpen ? '收起规则详情' : '查看规则详情' }}</button></div><div class="evidence-grid"><div><strong>{{ selectedIssue === 'assignments' ? '3 个班级' : selectedIssue === 'conflict' ? '2 个班级' : '1 个科目' }}</strong><small>受影响对象</small></div><div><strong>{{ selectedIssue === 'unscheduled' ? '14 节' : selectedIssue === 'assignments' ? '8 项' : '2 条' }}</strong><small>待处理数量</small></div><div><strong>{{ selectedIssue === 'conflict' ? '周二第 3 节' : '自动排课前' }}</strong><small>建议处理时机</small></div></div><div v-if="advancedRulesOpen" class="advanced-rule-box"><strong>相关规则</strong><span>教师可排时段、场地容量、固定课位</span><span>点击任意规则可回到对应设置页</span></div></div>
            ...
            <aside class="release-sidebar" aria-label="发布状态">
              ...
              <ol class="check-list"><li :class="{ 'is-done': state.assignmentReady === state.assignmentTotal }"><span><Check v-if="state.assignmentReady === state.assignmentTotal" :size="14" /><span v-else>1</span></span><div><strong>教学任务完整</strong><small>{{ assignmentLabel }}</small></div></li><li :class="{ 'is-done': state.preflightErrors === 0 }"><span><Check v-if="state.preflightErrors === 0" :size="14" /><span v-else>2</span></span><div><strong>前置检查</strong><small>{{ state.preflightErrors ? `${state.preflightErrors} 个错误` : '无阻断错误' }}</small></div></li><li :class="{ 'is-done': state.remaining === 0 }"><span><Check v-if="state.remaining === 0" :size="14" /><span v-else>3</span></span><div><strong>未排课时</strong><small>{{ state.remaining ? `剩余 ${state.remaining} 节` : '全部安排' }}</small></div></li><li :class="{ 'is-done': state.draftStatus === '已发布' }"><span><Check v-if="state.draftStatus === '已发布'" :size="14" /><span v-else>4</span></span><div><strong>通知与审计</strong><small>{{ state.draftStatus === '已发布' ? '已生成站内通知' : '发布后自动生成' }}</small></div></li></ol>
        ...
        <section v-else class="prototype-variant variant-c" data-testid="variant-c">
          <header class="variant-page-head">
            ...
            <div class="variant-head-actions"><span class="canvas-view-select"><Table2 :size="14" /> 班级视角 <ChevronDown :size="14" /></span><button type="button" class="button button-secondary" @click="openPublishPanel"><FileCheck2 :size="15" /> 发布检查</button></div>
          </header>

          <div class="canvas-toolbar"><div class="canvas-toolbar-left"><button type="button" class="toolbar-chip is-active" @click="record('保持班级视角', 'neutral')">班级</button><button type="button" class="toolbar-chip" @click="record('切换到教师视角', 'neutral')">教师</button><button type="button" class="toolbar-chip" @click="record('切换到场地视角', 'neutral')">场地</button><span class="toolbar-divider" /><span class="toolbar-context">高一 1 班 <ChevronDown :size="14" /></span></div><div class="canvas-toolbar-right"><span class="toolbar-summary"><i class="legend-dot is-blue" /> 已排 170 <i class="legend-dot is-orange" /> 待处理 {{ state.remaining }}</span><button type="button" class="icon-inline-button" title="筛选课程" aria-label="筛选课程" @click="record('打开课程筛选', 'neutral')"><Search :size="16" /></button><button type="button" class="icon-inline-button" title="更多画布设置" aria-label="更多画布设置" @click="record('打开画布设置', 'neutral')"><SlidersHorizontal :size="16" /></button></div></div>
          ...
          <div class="canvas-layout">
            ...
            <aside class="canvas-inspector" aria-label="课程检查面板">
              ...
              <div class="inspector-section"><div class="inspector-section-head"><span>检查结果</span><span class="issue-pill">{{ state.conflicts }} 个问题</span></div><div class="inspector-alert"><CircleAlert :size="15" /><div><strong>机房资源冲突</strong><small>周二第 3 节，另一个班也需要机房。</small></div></div><button type="button" class="button button-secondary button-block" @click="record('打开关联场地课表', 'blue')"><Table2 :size="15" /> 查看场地视角</button></div>
...
.flow-rail { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); margin-bottom: 16px; border: 1px solid var(--p-line); border-radius: 7px; background: #fff; overflow: hidden; }
...
.prototype-switcher { position: fixed; z-index: 60; bottom: 16px; left: 50%; display: flex; width: min(390px, calc(100vw - 24px)); min-height: 54px; align-items: center; gap: 8px; padding: 7px 9px; transform: translateX(-50%); border: 1px solid #263a56; border-radius: 8px; background: #1c2a3d; color: #fff; box-shadow: 0 10px 28px rgba(13, 26, 45, .26); }.prototype-switcher-arrow { display: grid; width: 34px; height: 34px; flex: 0 0 auto; place-items: center; border: 1px solid #4e6380; border-radius: 5px; background: transparent; color: #fff; cursor: pointer; }.prototype-switcher-arrow:hover { background: #2a3d58; }.prototype-switcher-label { display: grid; min-width: 0; flex: 1; gap: 1px; text-align: center; }.prototype-switcher-label span { color: #aebdd0; font-size: 9px; letter-spacing: .06em; text-transform: uppercase; }.prototype-switcher-label strong { overflow: hidden; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }.prototype-switcher-label small { overflow: hidden; color: #c4cfdd; font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
...
@media (max-width: 680px) {
  .prototype-app { display: block; }.prototype-sidebar { display: none; }.prototype-main { min-height: 100vh; }.prototype-menu-button { display: inline-flex; }.prototype-topbar { min-height: 60px; padding-inline: 11px; }.prototype-breadcrumb > span:first-child, .prototype-dev-badge, .prototype-semester, .prototype-user > span:last-child { display: none; }.prototype-topbar-actions { gap: 6px; }.prototype-content { padding: 14px 11px 90px; }.prototype-context-line { align-items: flex-start; flex-direction: column; gap: 6px; }.flow-rail { grid-template-columns: repeat(5, minmax(74px, 1fr)); overflow-x: auto; }.flow-step { min-width: 90px; min-height: 59px; padding: 8px; }.flow-step > span:nth-child(2) { display: none; }.flow-step-index { margin: 0 auto; }.flow-step-arrow { display: none; }.command-panel { padding: 18px; }.command-panel h2 { font-size: 21px; }.command-trust-row { display: grid; gap: 7px; }.mini-timetable { overflow-x: auto; }.mini-timetable-head, .mini-timetable-row { min-width: 430px; }.preview-foot .panel-link-button { width: 100%; margin-left: 0; }.inbox-layout { display: block; }.issue-inbox { border-right: 0; border-bottom: 1px solid var(--p-line); }.issue-row { display: inline-grid; width: calc(50% - 4px); vertical-align: top; }.inbox-progress { margin-top: 13px; }.issue-detail { padding: 18px 14px; }.release-sidebar { display: grid; grid-template-columns: 1fr 1fr; padding: 16px 14px; }.release-heading, .check-list, .release-note { grid-column: 1 / -1; }.check-list { grid-template-columns: 1fr 1fr; }.release-sidebar .button-block { width: 100%; }.evidence-grid { grid-template-columns: 1fr; }.detail-grid { min-width: 410px; overflow-x: auto; }.canvas-toolbar { align-items: flex-start; flex-direction: column; }.canvas-toolbar-right { width: 100%; justify-content: space-between; }.canvas-selection { margin-left: 0; }.canvas-inspector { display: block; }.inspector-section { margin-top: 17px; }.inspector-score { margin-top: 18px; }.prototype-state-strip { align-items: flex-start; flex-direction: column; gap: 8px; }.state-strip-facts { gap: 7px; }.prototype-switcher { bottom: 10px; }
}

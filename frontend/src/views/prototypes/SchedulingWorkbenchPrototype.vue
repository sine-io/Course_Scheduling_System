      ...
      <div class="swp-content">
        ...
        <section v-if="currentVariant === 'A'" class="swp-variant swp-variant-a" data-testid="prototype-variant-a">
          ...
          <nav class="swp-step-rail" aria-label="排课流程">
            <button v-for="(railStep, index) in steps" :key="railStep.key" class="swp-step-rail-item" :class="{ 'is-active': selectedStep === railStep.key, 'is-done': railStep.state === 'done', 'is-blocked': railStep.state === 'blocked' }" type="button" :data-testid="`prototype-step-a-${railStep.key}`" @click="selectStep(railStep.key)">
              <span class="swp-step-index"><Check v-if="railStep.state === 'done'" :size="14" /><span v-else>{{ index + 1 }}</span></span>
              <span><strong>{{ railStep.label }}</strong><small>{{ railStep.summary }}</small></span>
        ...
        <section v-else-if="currentVariant === 'B'" class="swp-variant swp-variant-b" data-testid="prototype-variant-b">
          ...
          <div class="swp-b-layout">
            <aside class="swp-b-rail" aria-label="排课步骤导航"><div class="swp-b-rail-title"><span>排课步骤</span><small>按当前数据推导</small></div><button v-for="(railStep, index) in steps" :key="railStep.key" class="swp-b-step" :class="{ 'is-active': selectedStep === railStep.key, 'is-done': railStep.state === 'done', 'is-blocked': railStep.state === 'blocked' }" type="button" :data-testid="`prototype-step-b-${railStep.key}`" @click="selectStep(railStep.key)"><span class="swp-b-step-number"><Check v-if="railStep.state === 'done'" :size="14" /><span v-else>{{ index + 1 }}</span></span><span><strong>{{ railStep.label }}</strong><small>{{ railStep.summary }}</small></span><ArrowRight :size="14" /></button><div class="swp-b-rail-foot"><span class="swp-legend-dot is-warning" /> 2 项待处理提醒</div></aside>

...
test.describe('生产应用壳层', () => {
  ...
  for (const viewport of viewports) {
    test(`${viewport.width}x${viewport.height} 使用稳定侧栏且页面不横向溢出`, async ({ page }) => {
      ...
      await expect(sidebar.locator('.app-sidebar-footer')).toBeVisible()
      await expect(sidebar.locator('.app-school-copy strong')).toHaveCount(1)
      await expect(page.getByTestId('shell-school-context')).toHaveCount(0)
